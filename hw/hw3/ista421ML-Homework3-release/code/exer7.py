...
for i in orders:
    ...
    #for k in range(num_function_samples):
     #   x,t = sample_from_function(25, noise_var, xmin, xmax )
    ...
    #print true function
    myx = np.arange(-12, 5, 0.2)
    plt.plot(myx, 1 + (0.1 * myx) + (0.5 * np.power(myx,2)) + \
        (0.05 * np.power(myx,3)), color='r', linewidth=3)
    ...
    ti = 'Plot of {0} functions where parameters '\
         ...
         .format(num_function_samples, i)
    ti = '20 Best fit models - polynomial order ' + str(i)     
    plt.title(ti)
    plt.pause(.1) # required on some systems so that rendering can happen
